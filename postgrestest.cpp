...
int main()
{
    try
    {
        ...
        pqxx::connection c("dbname=northwind user=postgres password=1337 hostaddr=127.0.0.1 port=5432");
        ...
        for (auto [name, salary] : tx.query<std::string, int>(
            "SELECT first_name, employee_id FROM employees ORDER BY employee_id"))
        {
